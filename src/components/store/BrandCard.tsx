        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="heading-sm text-text-primary mb-1 line-clamp-1">
              <Link
                href={brandHref}
                className="hover:text-roh-accent-orange transition-colors"
              >
                {brand.name}
              </Link>
            </h3>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h4 className="font-semibold text-text-primary">
            {t("product.featuredSauces")}
          </h4>
          <span className="text-xs text-text-muted">
            {productCount}{" "}
            {productCount === 1 ? t("product.sauce") : t("product.sauces")}
          </span>
        </div>
                ...
                <div key={product.id} className="relative">
                  ...
                  {product.heatLevel && (
                    <div className="absolute -top-1 -right-1 bg-white rounded-full p-1 shadow-sm">
                      <div
                        className={`w-3 h-3 rounded-full ${heatColor(product.heatLevel)}`}
                      />
                    </div>
                  )}
                </div>
                  ...
                  >
                    ...
                    {product.heatLevel && (
                      <div className="flex items-center gap-1 ml-2">
                        <div
                          className={`w-2 h-2 rounded-full ${heatColor(product.heatLevel)}`}
                        />
                        <span className="text-xs text-text-muted">
                          {product.heatLevel}
                        </span>
