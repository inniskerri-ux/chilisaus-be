...
export function getProductWeight(item: OrderItem): number {
  ...
  switch (productType) {
    case "bottle": {
      ...
      const bottleWeight =
        PRODUCT_WEIGHTS.bottles[
          capacity as keyof typeof PRODUCT_WEIGHTS.bottles
        ];
    ...
    default:
      ...
      console.warn(
        `[Shipping] Unknown product type for "${item.productName}", using default weight`,
      );
